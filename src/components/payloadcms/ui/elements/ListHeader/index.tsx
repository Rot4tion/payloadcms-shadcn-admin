...
export const ListHeader: React.FC<ListHeaderProps> = (props) => {
  return (
    <header className={cn('flex items-center flex-wrap [&_.btn]:m-0', props.className)}>
      <div className="grid grid-cols-[1fr_auto] w-full">
        <div className="flex flex-wrap items-center gap-[calc(var(--base)*0.5)]">
          <h1 className="m-0">{props.title}</h1>
          {props.TitleActions.length ? (
            <div className="flex gap-[calc(var(--base)*0.5)]">{props.TitleActions}</div>
          ) : null}
        </div>
